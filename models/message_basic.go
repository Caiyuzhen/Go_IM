...
func Chat(writer http.ResponseWriter, request *http.Request) {
	...
	// 【第八步】把在线用户的消息加到 Redis 缓存中
	// SetUserOnlineInfo("online_"+Id, []byte(node.Addr), time.Duration(viper.GetInt("timeout.RedisOnlineTime"))*time.Hour)
	SetUserOnlineInfo("online_"+Id, []byte(node.Addr), time.Duration(4)*time.Hour)
...
func sendMsg_Podcast(userId int64, msg []byte) { // 传入 userId 和 msg
	...
	var key string
	if userId > jsonMsg.UserId { // 表示消息的递增 (确定两个用户 ID 的顺序, 如果你有两个用户ID，1 和 2，不管是用户1发送给用户 2 的消息, 还是用户 2 发送给用户 1 的消息, 都希望它们能够存储在同一个 Redis 键下)
