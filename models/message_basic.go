...
type MessageBasic struct {
	...
	Type     int    // 消息类型 (群聊、私聊、广播(比如欢迎加入 XXX 群聊))  => 用 1 2 3 来表示
	Media    int    // 消息媒体类型 (文本、图片、音频、视频、文件)
	Content  string // 消息内容
	Pic      string // 图片地址
	Audio    string // 音频地址
	Url      string // 链接地址
...
// 🔥🔥 聊天室的总的公共方法(处理客户端连接请求的函数, 当客户端尝试建立 WebSocket 连接时会创建一个 Node 实例, 将其添加到 clientMap 中, 并启动发送（sendProc）和接收（receiveProc）协程) => 单聊、群聊、广播都需要获取一些参数等等 -> 发送消息, 需要 【发送者 ID】、【接收者 ID】、【消息类型】、【消息内容】
...
/* 
	sendProc_websocketMsg_Personal   		将消息从服务端发送给特定的客户端 (私聊消息), 监听每个连接的客户端的 DataQueue, 并将队列中的消息通过 WebSocket 发送给客户端
	receiveProc_websocketMsg_Personal 		接收来自客户端的 WebSocket 消息, 当服务端从客户端接收到消息后, 可以根据业务逻辑处理这些消息, 比如将收到的消息广播给其他用户或处理特定的请求
	...
	sendMsg_Podcast 						🌟 发送【📢 广播消息】的具体方法, 将消息发送给特定的客户端或者一组客户端

*/

// 👇 发送消息的具体方法 ——————————————————————————————————————————————————————————————————————————————————————————————————————————————
// 发送【 💬 websocketMsg_Persona 双向消息】的方法 (从管道中取出数据)
func sendProc_websocketMsg_Personal(node *Node) {
	for {
		...
		case data := <-node.DataQueue: // 从管道中获取数据 🔥
			...
			fmt.Println("✅ 发送消息成功", string(data))
		}
	}
}

// 接收【 💬 websocketMsg_Persona 双向消息】的方法（发送方也会接收到自己发送的消息, 接收到数据后可以广播给其他地方）
func receiveProc_websocketMsg_Personal(node *Node) {
	for {
		...
		fmt.Println("✅ 接收(自己发送的)消息成功", string(data))
	}
}
...
// Go 语言会在程序启动时自动执行该（🌟 初始化函数）。在这里，它用于启动处理 UDP 📢 广播消息发送（udpSendProc）和 📢接收广播消息（udpReceiveProc）的协程（数据的发送与接收读取） ——————————————————————————————————————————————————————————————————————————————————————————————————————————————
func init() {
...
// 🌟 发送【📢 广播消息】的具体方法, 将消息发送给特定的客户端或者一组客户端
...
// 🌟 【📢 广播消息】的调度逻辑 => 判断要把拿到的局域网消息分发消息到【单聊】、【群聊】还是【系统消息】等, 看业务需求******************************************************************
func dispatchMsg_Podcast(data []byte) {
	...
	if utils.IsValidJSON(data) {
		...
		if err != nil {
			...
			fmt.Println("解析静态JSON成功", msg)
		}

		// 根据消息类型进行分发
		switch msg.Type {
		case 1: // 私聊
			sendMsg_Podcast(msg.TargetId, data)
			// case 2: // 群聊
			// sendGroupMsg(msg)
			// case 3: // 广播
			// sendAllMsg(msg)
		...
		dataStr := string(data)
		// fmt.Println("🤔 接收到的数据不是有效的 JSON 格式: ", dataStr)
		msg := MessageBasic{}             // 初始化消息结构体

		// 根据消息类型进行分发
		switch msg.Type {
			case 1: // 私聊
				sendMsg_Podcast(msg.TargetId, []byte(dataStr))
				// case 2: // 群聊
				// sendGroupMsg(msg)
				// case 3: // 广播
				// sendAllMsg(msg)
